...
@Component({
  ...
  imports: [CommonModule, ModalForm, BookTabs, LessonForm],
...
export class LessonList implements OnInit {
  ...
  loadItems() {
    ...
    // Call API
    this.bookService.list('lessons', query).then((res: any) => {
  ...
  navigateToEdit(id: number) {
    ...
    const path = `${buildPath(PATH.admin.book)}/${bId}/${buildPath(PATH.admin.lesson.edit).replace(':id', id.toString())}`; 
  ...
  handleFormSubmit(data: any) {
    ...
    this.bookService.create('lessons', data).then(() => {
  ...
  deleteItem(id: number) {
    this.alertService.delete('Eliminar', '¿Estás seguro?', () => {
      this.loading.set(true);
      this.bookService.delete('lessons', id).then(() => {
